-- 20260113000000_add_auto_quota_cache_refresh.sql
-- Purpose: Auto-refresh user_quotas cache after consume_quota and refund_quota
-- Problem: user_quotas cache was not updating in real-time, causing stale quota display
-- Solution: Call refresh_user_quota() at the end of consume/refund operations

--------------------------------------------------------------------------------
-- Update consume_quota to auto-refresh cache
--------------------------------------------------------------------------------
...
BEGIN
  ...
  IF v_event_id IS NULL THEN
    ...
    -- No need to refresh cache if this is a duplicate call
  ...
  -- 5. Auto-refresh user_quotas cache for real-time updates
...
--------------------------------------------------------------------------------
-- Update refund_quota to auto-refresh cache
--------------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.consume_quota IS 
  'Consumes quota from grants with automatic cache refresh. Modified 2026-01-13 to add real-time cache updates.';

COMMENT ON FUNCTION public.refund_quota IS 
  'Refunds consumed quota back to original grants with automatic cache refresh. Modified 2026-01-13 to add real-time cache updates.';
