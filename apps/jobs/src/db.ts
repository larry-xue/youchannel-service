...
export async function updateYoutubeAccountTokens(
  ...
  if (params.accessToken !== undefined) {
    updates.push(`access_token = $${index++}`);
    values.push(params.accessToken);
  }
  if (params.refreshToken !== undefined) {
    updates.push(`refresh_token = $${index++}`);
  ...
  if (params.scope !== undefined) {
    updates.push(`scope = $${index++}`);
    values.push(params.scope);
  }
  if (params.tokenType !== undefined) {
    updates.push(`token_type = $${index++}`);
    values.push(params.tokenType);
  }
  ...
  const result = await pool.query(
    `update youtube_accounts set ${updates.join(", ")}
     where id = $${index}
     returning id`,
    values
  );
