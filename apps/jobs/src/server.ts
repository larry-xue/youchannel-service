...
export async function buildServer(params: {
  ...
  // PG-Boss Job Monitoring endpoints

  // GET /admin/jobs - Queue statistics overview
  app.get("/admin/jobs", { preHandler: requireAdmin }, async () => {
    const result = await db.query<{ name: string; state: string; count: string }>(
  ...
  // GET /admin/jobs/list - Paginated job list
  app.get("/admin/jobs/list", { preHandler: requireAdmin }, async (request, reply) => {
    const query = request.query as Record<string, string | string[] | undefined>;
